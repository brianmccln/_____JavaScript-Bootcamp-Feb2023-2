...
function respondToKeyPress(event) {
  ...
  if(event.key === 'c' || event.key === 'Enter') { // rand bg color
  ...
  } else if(event.key === 'd') { // toggle dark mode
    if(!dark) { // same as: if(dark == false)
  ...
  } else if(event.key === 'f') { // toggle font
    if(serif) {
  ...
  // 11. If the `p` or `n` key is pressed, generate and output a random 4-digit pin number. If r is less than 4 digits, add a leading 0; output the random pin
  } else if(event.key === 'p' || (event.key === 'n')) { // rand pin num
    ...
    } else if(event.keyCode === 37) { // move space ship left
      if(leftPos > 0) {
        leftPos -= shipSpeed;
        spaceShip.style.left = leftPos + 'px';
      }
    ...
    } else if(event.key === 'ArrowRight') { // move space ship right
