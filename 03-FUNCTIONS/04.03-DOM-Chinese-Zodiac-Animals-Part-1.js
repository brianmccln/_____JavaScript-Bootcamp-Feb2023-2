        ...
        tbl += `<tr>
                    ...
                    <td>
                        <img src="images/sound-icon.png" width="35px" height="auto"          
                                onclick="playSound()" id="${arr[i].eng}" style="cursor:pointer">
                    </td>
                    <td>
...
function calculateYears(year) {
    ...
    for(let i = startYr; i <= endYr; i+=12) {
        if(i == year) { // if it's the active year, make it red
            years += '<span style="color:red">' + i + '</span>, '
        } else if(i == endYr) { // if last year don't add a comma
            years += i 
        } else { // if it's neither of the above, add a comma
            years += i + ", "
        }
